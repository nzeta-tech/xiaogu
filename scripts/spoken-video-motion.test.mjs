...
test("photo request preserves narration and voice with a short restrained motion direction", () => {
  ...
  assert.match(request.motion_prompt, /restrained facial expressions/);
  assert.match(request.motion_prompt, /hands in their original resting position/);
  assert.ok(request.motion_prompt.length < 160);
