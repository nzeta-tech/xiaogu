...
export async function researchSegmentsWithCodex(segments,dir,runner=runCodex,resume=null){
  if(resume){
    const batchSize=Math.max(1,Math.min(8,Number(process.env.LOCAL_AGENT_VIDEO_RESEARCH_BATCH_SIZE)||4));
    const concurrency=Math.max(1,Math.min(4,Number(process.env.LOCAL_AGENT_VIDEO_RESEARCH_CONCURRENCY)||2));
    const results=new Array(segments.length),missing=[];
    for(let index=0;index<segments.length;index++){
      const segment=segments[index];
      const input={id:segment.id,text:segment.text,visual:segment.visual};
      const validate=value=>Array.isArray(value)&&value.length===1&&value[0]?.id===segment.id&&validSources(value[0].sources);
      const reported=await resume.get("web-research-v1",input,validate);
      if(reported)results[index]=normalizeWebSources(reported[0].sources,segment).slice(0,3);
      else missing.push({segment,index,input,validate});
    }
    ...
    await mapVideoWork(batches,concurrency,async(batch,batchIndex)=>{
      const batchDir=path.join(dir,`research-batch-${batchIndex}`);
      await mkdir(batchDir,{recursive:true});
      const reported=await measureVideoStage("web-research-batch-v1",()=>researchReportedSegments(batch.map(item=>item.segment),batchDir,runner),{segments:batch.length});
      for(let offset=0;offset<batch.length;offset++){
        const item=batch[offset],entry=reported[offset],value=[entry];
        await resume.put("web-research-v1",item.input,value,item.validate);
        results[item.index]=normalizeWebSources(entry.sources,item.segment).slice(0,3);
      }
    });
    return results;
  }
