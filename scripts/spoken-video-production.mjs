...
async function download(url,file,headers={}) {
  ...
  if(socksProxy&&!Object.keys(headers).length&&!/^(?:localhost|127\.0\.0\.1)$/.test(hostname)){
    ...
    await run("curl",args,{timeout:190000}).catch(()=>{throw new Error("素材下载失败：外部素材源不可达");});
  ...
  if(declared&&!response.headers.get("content-encoding")&&(await stat(file)).size!==declared)throw new Error("素材下载不完整，请重试");
...
export async function planMaterials(dir,script,template) {
  ...
  const prompt="Read research-input.json. Keep the Chinese spoken script unchanged. Use the selected template's structure as pacing and scene reference when present. Create material-plan.json as JSON with a segments array. Each segment must have id, text, query (2-5 concrete English visual search terms for relevant licensed photos or video), and visual (a concise audience-facing Chinese title of at most 18 characters; never mention 口播, 知识卡, 分镜, 原文, or production workflow in the title). Also provide cardPoints: 2-3 verbatim excerpts from that segment, each 4-72 Chinese characters, preserving complete conditions, negation, comparisons and numbers. Order cardPoints by teaching priority: the first must be the one takeaway a viewer should remember; the others support or explain it. The title must accurately cover those exact cardPoints: if one point introduces a question and the others give data, title the question-and-data relationship; never promise a comparison or two choices unless the points actually make that comparison. These explain the segment when no external media matches. Keep the same segment ids and text. Do not invent facts or numbers. Output only the file.";
  try {
    const env={...process.env};delete env.HEYGEN_API_KEY;
    await run(process.env.CODEX_CLI_BIN||"codex",["exec","--model",process.env.CODEX_CLI_MODEL||"gpt-5.6-terra","--skip-git-repo-check","--sandbox","workspace-write",prompt],{cwd:dir,env,timeout:180000});
    const parsed=JSON.parse(await readFile(path.join(dir,"material-plan.json"),"utf8"));
    if(Array.isArray(parsed.segments)&&parsed.segments.length===fallback.length&&parsed.segments.every((s,i)=>s.id===fallback[i].id&&s.text===fallback[i].text))return parsed.segments.map(s=>({...s,query:safe(s.query).slice(0,90)||"family planning",visual:safe(s.visual).slice(0,80)}));
  } catch(error) { throw new Error(`Codex 素材规划失败：${error instanceof Error?error.message:String(error)}`); }
  throw new Error("Codex 未能生成与口播文案逐段对应的素材方案");
}
...
export async function createKnowledgeCard(segment,dir,index,options={}){
  ...
  if(!wide&&options.presentation!=="editorial")return createProfessionalKnowledgeCard({...segment,visual:title,cardPoints:points},knowledgeCardSpec(points,title,index),options.backgroundFile||null,dir,index);
...
export function knowledgeCardSpec(points,title,index){
...
async function heygenCreate(ctx,task,dir,person,leaseToken){
  ...
  const request={type:person.type,title:safe(p.title)||"口播视频",aspect_ratio:p.aspectRatio==="16:9"?"16:9":"9:16",caption:{file_format:"srt"}};
  if(person.type==="avatar")request.avatar_id=person.avatarId;
  else{const upload=cliData(await run(bin,["asset","create","--file",person.file],{env,timeout:180000}));const assetId=safe(upload.asset_id||upload.id);if(!assetId)throw new Error("HeyGen 未返回照片素材 ID");request.image={type:"asset_id",asset_id:assetId};}
  request.script=safe(p.script);request.voice_id=safe(p.voiceId);
...
async function renderSegment(master,material,maskFile,out,start,length,width,height,pipSize,showMaterial){
  ...
  if(showMaterial){
    ...
    const y=height-pipSize-(height>width?80:100);
...
export function compactSrt(input,maxChars=14){
  ...
  for(const entries of groups){
    ...
    if(cursor>0)pieces.splice(0,pieces.length,[0,n]);
...
export async function finalize(master,segments,materials,subtitleUrl,script,dir,title,aspectRatio,options={}){
  ...
  const compact=compactSrt(await readFile(srt,"utf8"),Number.isInteger(options.subtitleMaxChars)?Math.min(14,Math.max(10,options.subtitleMaxChars)):14);if(!compact.trim())throw new Error("字幕时间轴无效");await writeFile(srt,compact);
  ...
  for(let i=0;i<segments.length;i++){
    ...
    const shots=[{start:cursor,length:presenterLength,showMaterial:false},{start:cursor+presenterLength,length:len-presenterLength,showMaterial:true}].filter(shot=>shot.length>=.5);
  ...
  for(const [index,shot] of timedShots.entries()){
    ...
    await renderSegment(master,shot.material,maskFile,output,shot.start-lead,shot.length+lead+tail,width,height,pipSize,shot.showMaterial);
    pieces.push(output);
  }
  ...
  if(transitionSeconds&&pieces.length>1){
    ...
    await run("ffmpeg",["-y",...pieces.flatMap(file=>["-i",file]),"-i",master,"-filter_complex",filters.join(";"),"-map","[joined]","-map",`${pieces.length}:a:0`,"-c:v","libx264","-preset","veryfast","-crf","20","-pix_fmt","yuv420p","-r","30","-c:a","aac","-b:a","192k","-t",total.toFixed(3),"-movflags","+faststart",stitched],{timeout:1800000});
  }else await run("ffmpeg",["-y","-f","concat","-safe","0","-i",list,"-c","copy",stitched],{timeout:300000});
  ...
  await run("ffmpeg",["-y","-i",stitched,"-i",titleCard,"-filter_complex",`[0:v][1:v]overlay=0:0:enable='lt(t,${options.showTitle===false?0:options.titleDuration??4.5})'[titled];[titled]${subtitleFilter}[v]`,"-map","[v]","-map","0:a","-c:v","libx264","-preset","veryfast","-crf","20","-pix_fmt","yuv420p","-c:a","aac","-b:a","192k","-movflags","+faststart",output],{timeout:1800000});
...
async function checkedFinalVideo(final){
  ...
  const captions=(await readFile(final.subtitleFile,"utf8")).trim().split(/\n\n/);
  if(!captions.length||captions.some(cue=>Array.from(cue.split("\n").slice(2).join("")).length>14))throw new Error("字幕超过画面安全字数");
  await run("ffmpeg",["-v","error","-i",final.output,"-f","null","-"],{timeout:300000});
}

export async function renderWithCodexReview({master,segments,materials,subtitleUrl,script,dir,title,aspectRatio,references=[],onProgress=async()=>{},generateVisual:generateVisualForReview=null,initialOptions={},resolveMaterial=null}){
  ...
  for(let attempt=1;attempt<=3;attempt++){
    ...
    const final=await finalize(master,currentSegments,currentMaterials,subtitleUrl,script,dir,title,aspectRatio,options);
    await checkedFinalVideo(final);
    const sheet=await createReviewSheet(final.output,dir,currentSegments,final.durationSeconds,Number.isFinite(options.presenterShare)?options.presenterShare:.65);
    const review=await codexReview({dir,contactSheet:sheet,title,script,segments:currentSegments,materials:currentMaterials,references,attempt,presenterShare:Number.isFinite(options.presenterShare)?options.presenterShare:.4});
    reviewHistory.push({attempt,pass:review.pass,issues:review.issues});
    if(review.pass)return {...final,materials:currentMaterials,segments:currentSegments,reviewHistory,options};
    if(attempt===3)break;
    let changed=false;
    ...
    for(const [id,fixValue] of Object.entries(cardFixes)){
      ...
      const revised={...currentSegments[index],forceCard:true,cardStyle:style};
    ...
    for(const [id,queryValue] of Object.entries(review.searchQueries||{})){
      ...
      for(const query of queries){revised={...currentSegments[index],query:query.trim().slice(0,90)};replacement=resolveMaterial?await resolveMaterial(revised,index):await materialFor(revised,dir,index);if(!replacement.source.startsWith("xiaogu-"))break;}
    ...
    if(Number.isFinite(requestedShare)&&requestedShare>=.3&&requestedShare<=.8&&requestedShare!==options.presenterShare){options.presenterShare=requestedShare;changed=true;}
    ...
    if(!changed)throw new Error(`Codex 质检未通过，且没有可执行的修复：${review.issues.join("；")}`);
    await onProgress(`Codex 发现问题，正在按建议修订：${review.issues.slice(0,2).join("；")}`,Math.min(94,78+attempt*6));
  }
  throw new Error(`Codex 三轮质检仍未通过：${reviewHistory.at(-1)?.issues.join("；")||"画面质量不足"}`);
}
...
export async function executeSpokenVideoProduction(task,leaseToken,ctx){
  ...
  try {
    ...
    const segments=await planMaterials(dir,script,p.template||null);
    ...
    let created;
    if(resumeId) {
      await report(ctx,task,leaseToken,jobId,"heygen_rendering",34,"正在恢复已提交的口播视频，不会重复生成…");
      created=await heygenPoll(resumeId);
    } else {
      const person=await personInput(ctx,task,dir);
      await report(ctx,task,leaseToken,jobId,"heygen_submitting",20,"人物与声音已确定，正在提交一次 HeyGen 口播任务…");
    ...
    await report(ctx,task,leaseToken,jobId,"presenter_ready",50,"口播母版已完成，正在按实际时长匹配画面…");
    await report(ctx,task,leaseToken,jobId,"researching",52,productionMode==="smart"?"正在按口播内容搜集画面与参考资料…":"正在按分镜准备画面素材…");
    const references=productionMode==="smart"?await researchSegmentsWithCodex(segments,dir):segments.map(()=>[]);
    const materials=[];for(let i=0;i<segments.length;i++)materials.push(await productionMaterial(ctx,jobId,segments[i],dir,i,{aspectRatio:p.aspectRatio,forceCard:productionMode==="smart"&&shouldUseExplainerCard(segments[i]),researchReferences:references[i]}));
    const manifest=segments.map((segment,i)=>({id:segment.id,text:segment.text,visual:segment.visual,textReference:references[i][0]||null,researchReferences:references[i],material:{kind:materials[i].kind,title:materials[i].title,source:materials[i].source,license:materials[i].license,licenseUrl:materials[i].licenseUrl||"",credit:materials[i].credit||"",changes:materials[i].changes||""}}));
    await report(ctx,task,leaseToken,jobId,"materials_ready",68,`已为 ${segments.length} 段口播准备画面素材`,{materialPlan:manifest});
    await report(ctx,task,leaseToken,jobId,"composing",72,"Codex 正在主导混剪，并检查画面与字幕…");
    const final=await renderWithCodexReview({master,segments,materials,subtitleUrl:created.subtitleUrl,script,dir,title,aspectRatio:p.aspectRatio,references,onProgress:(message,progress)=>report(ctx,task,leaseToken,jobId,"quality_check",progress,message),generateVisual:(segment,index)=>generateVisual(ctx,jobId,segment,dir,index),resolveMaterial:(segment,index)=>productionMaterial(ctx,jobId,segment,dir,index,{aspectRatio:p.aspectRatio,forceCard:segment.forceCard,cardStyle:segment.cardStyle,researchReferences:references[index]})});
    await report(ctx,task,leaseToken,jobId,"quality_check",96,"Codex 已通过成片验收，正在保存文件…");
    ...
    return {status:"completed",jobId,productionMode,videoUrl,coverUrl,durationSeconds:final.durationSeconds,presenterMasterUrl:masterUrl,materialPlan:finalManifest,qualityReview:final.reviewHistory,subtitleSrt:await readFile(final.subtitleFile,"utf8"),editOptions:final.options||{},creativeSummary:[`画面素材 ${final.materials.length} 段：外部 ${externalCount}、AI 生成 ${generatedCount}、本地 ${final.materials.length-externalCount-generatedCount}`,"已完成一次 HeyGen 口播合成",`Codex 已完成 ${final.reviewHistory.length} 轮成片验收`,`已完成混剪、字幕、标题和封面`]};
...
async function productionMaterial(ctx,jobId,segment,dir,index,options={}){
  ...
  try {
    const spec=knowledgeCardSpec(knowledgePoints(segment),segment.visual,index);
...
export async function planRecut(dir,input){
  const segments=input.materialPlan.map((s,i)=>({id:s.id||`s${i+1}`,text:s.text,visual:s.visual,query:s.query||s.material?.query||"",cardPoints:s.material?.points||[],material:s.material}));
...
export async function executeSpokenVideoRecut(task,leaseToken,ctx){
  ...
  try{
    ...
    const plan=await planRecut(dir,input);
    if(productionMode==="smart"&&input.baseProductionMode!=="smart")for(const segment of plan.segments)segment.regenerate=true;
    ...
    const changedSegments=productionMode==="smart"?plan.segments.filter(segment=>segment.regenerate):[];
    ...
    return {status:"completed",jobId,productionMode,videoUrl,coverUrl,durationSeconds:final.durationSeconds,subtitleSrt:await readFile(final.subtitleFile,"utf8"),editOptions:editOptions(final.options,input.aspectRatio),materialPlan:final.segments.map((s,i)=>({id:s.id,text:s.text,visual:s.visual,query:s.query,textReference:references[i][0]||null,researchReferences:references[i],material:{kind:archived[i].kind,title:archived[i].title,source:archived[i].source,license:archived[i].license,licenseUrl:archived[i].licenseUrl||"",credit:archived[i].credit||"",changes:archived[i].changes||"",points:archived[i].points||[],mediaId:archived[i].mediaId}})),qualityReview:final.reviewHistory,creativeSummary:["已按修改要求完成新版本","保留原口播与声音","旧版本仍可查看和下载"]};
  }finally{await rm(dir,{recursive:true,force:true});}
}
