...
export async function createExpressionMaterial(segment,dir,index,options={}){
  ...
  const blocks=cells.map((c,i)=>{const label=labels[i]||`要点 ${i+1}`;return `<g filter="url(#shadow)"><rect x="${c.x}" y="${c.y}" width="${c.width}" height="${c.height}" rx="22" fill="${fills[i%4]}" stroke="${colors[i%4]}" stroke-width="2"/><rect x="${c.x}" y="${c.y}" width="${c.width}" height="12" rx="6" fill="${colors[i%4]}"/></g><rect x="${c.x+padding}" y="${c.y+30}" width="${Math.max(116,label.length*29)}" height="48" rx="24" fill="${colors[i%4]}"/><text x="${c.x+padding+18}" y="${c.y+64}" font-size="25" font-weight="700" fill="#fff">${label}</text>${c.lines.map((line,j)=>`<text x="${c.x+padding}" y="${c.y+112+font+j*lineHeight}" font-size="${font}" font-weight="600" fill="#172f38">${xml(line)}</text>`).join("")}`}).join("");
  ...
  await sharp(Buffer.from(fitCardSvg(svg,width,height))).jpeg({quality:94}).toFile(file);
  return {kind:"image",file,contentBounds:cardContentBounds(svg,width,height),source:"xiaogu-knowledge-card-expression",license:"project-owned",title:segment.visual||"",query:segment.query||"",points:spec.nodes,presentation:`expression-v2:${spec.kind}:${spec.grounding}:${pipSafe?"pip-safe":"fullscreen"}`};
}
