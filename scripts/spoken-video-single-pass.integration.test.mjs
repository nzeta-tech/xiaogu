...
test('smart presenter overlay keeps the presenter visible behind an animated material panel',{timeout:120000},async()=>{
  const dir=await mkdtemp(path.join(os.tmpdir(),'presenter-overlay-'));
  try{
    const master=path.join(dir,'master.mp4'),material=path.join(dir,'material.jpg'),srt=path.join(dir,'source.srt');
    await exec('ffmpeg',['-v','error','-y','-f','lavfi','-i','color=red:s=360x640:r=30','-f','lavfi','-i','sine=frequency=440:sample_rate=48000','-t','3','-c:v','libx264','-preset','ultrafast','-c:a','aac',master]);
    await sharp({create:{width:1080,height:1920,channels:3,background:'#0022ee'}}).jpeg().toFile(material);
    await writeFile(srt,'1\n00:00:00,000 --> 00:00:03,000\n浮窗素材\n');
    const result=await finalize(master,[{id:'overlay',text:'浮窗素材',intent:'scene',layout:'presenter-overlay'}],[{kind:'image',file:material,source:'fixture'}],'','浮窗素材',dir,'Fixture','9:16',{timelineMode:'semantic',subtitleFile:srt,showTitle:false,transitionSeconds:0});
    const {stdout}=await exec('ffmpeg',['-v','error','-ss','1.5','-i',result.output,'-frames:v','1','-f','image2pipe','-vcodec','png','-'],{encoding:'buffer',maxBuffer:10e6});
    const {data,info}=await sharp(stdout).removeAlpha().raw().toBuffer({resolveWithObject:true});
    const pixel=(x,y,channel)=>data[(y*info.width+x)*info.channels+channel];
    assert(pixel(180,500,2)>150,'material panel should be visible');
    assert(pixel(820,500,0)>150,'presenter background should remain visible');
    assert.equal(result.shotTimeline[0].layout,'presenter-overlay');
  }finally{await rm(dir,{recursive:true,force:true});}
});
