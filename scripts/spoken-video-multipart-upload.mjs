...
export async function uploadVideoParts({base,token,jobId,kind,file,size,name,contentType,cacheDir,fetchImpl=fetch,partBytes=1024*1024,concurrency=8}){
  ...
  const count=Math.ceil(size/partBytes);if(count>512||size>500*1024*1024)throw Error('媒体文件不能超过 500 MB');
  const cacheFile=cacheDir?path.join(cacheDir,`upload-${jobId}-${kind}-${partBytes}-${sha256}.json`):null;let saved={parts:{}};
  ...
  const call=async(url,options)=>{for(let attempt=0;attempt<3;attempt++){try{const response=await fetchImpl(url,{...options(),signal:AbortSignal.timeout(600000)});if(response.status>=500||response.status===429||response.status===408){await response.body?.cancel();throw Error(`分片上传暂不可用（${response.status}）`);}const result=await response.json();if(!response.ok){const error=new Error(`分片上传失败（${response.status}）`);error.permanent=true;throw error;}return result;}catch(error){if(error.permanent||attempt===2)throw error;await new Promise(r=>setTimeout(r,1000*(attempt+1)));}}};
