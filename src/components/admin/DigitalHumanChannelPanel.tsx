...
export function DigitalHumanChannelPanel({ settings, setSettings }: { settings: SystemSettings; setSettings: Dispatch<SetStateAction<SystemSettings>> }) {
  ...
  return <div className="pageStack">
    <section className="adminPanel"><div className="adminPanelHeader"><div><h2>数字人渠道</h2><p>创作者只看到统一的创建方式，供应商选择、停用和降级由这里控制。</p></div></div>
      ...
      <label className="adminField"><span>优先通道</span><select value={settings.digitalHuman.preferredProvider === "chanjing" ? "auto" : settings.digitalHuman.preferredProvider} onChange={event=>update({preferredProvider:event.target.value as SystemSettings["digitalHuman"]["preferredProvider"]})}><option value="auto">自动选择</option><option value="heygen">快速形象通道优先</option></select></label>
