...
export function AdminPageClient() {
  ...
  // These refs are read only by async callbacks and intentionally mirror the latest render values.
  ...
  async function inspectViralContentSource() {
    ...
    try {
      const response = await fetch(apiPath("/api/creation/link-remix/inspect"), { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ url: sourceUrl, adminOperation: true, metadataOnly: true, deferTranscription: true }) });
    ...
    <div className="adminConsole">
      ...
      {tab === "settings" ? (
        <div className="pageStack adminSettingsWorkspace">
          ...
          {settingsTab === "features" ? <AdminPanel title="本地 Agent 发布开关"><SettingsToggle title="爆款话题二创本地 Agent" hint="兼容 Agent 已在线后再开启；此开关会同时作用于全部 Web 节点。" checked={settings.features.localAgentEnabled} onChange={(checked) => setSettings((current) => ({ ...current, features: { ...current.features, localAgentEnabled: checked } }))} /></AdminPanel> : null}
