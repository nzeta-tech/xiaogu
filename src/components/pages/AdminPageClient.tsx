    ...
    <div className="adminConsole">
      ...
      {tab === "settings" ? (
        <div className="pageStack adminSettingsWorkspace">
          ...
          {settingsTab === "email" ? <AdminPanel title="低余额通知模板"><div className="settingsLegalEditor"><SettingsField label="邮件主题"><input value={settings.email.lowBalanceSubject} onChange={(event) => setSettings((current) => ({ ...current, email: { ...current.email, lowBalanceSubject: event.target.value } }))} /></SettingsField><SettingsField label="纯文本正文" hint="可用变量：{{name}}、{{balance}}、{{threshold}}、{{url}}"><textarea rows={8} value={settings.email.lowBalanceBody} onChange={(event) => setSettings((current) => ({ ...current, email: { ...current.email, lowBalanceBody: event.target.value } }))} /></SettingsField></div></AdminPanel> : null}
