...
export function XiaohongshuStudioPageClient({ app }: { app: CreationApp }) {
  ...
  async function download(card: Card, index: number) { const filename = `${title}-小红书${index === 0 ? "头图" : `章节配图-${index}`}.png`; if (isMobileDownloadDevice() && !card.url.startsWith("data:") && !card.url.startsWith("blob:")) { triggerDownload(buildImageDownloadUrl(card.url, filename), filename); return; } try { const source = card.url.startsWith("/") || card.url.startsWith("data:") || card.url.startsWith("blob:") || card.url.startsWith(window.location.origin) ? card.url : apiPath(`/api/assets/image-proxy?url=${encodeURIComponent(card.url)}`); const response = await fetch(source); if (!response.ok) throw new Error("图片下载失败"); const blob = await response.blob(); const url = URL.createObjectURL(blob); triggerDownload(url, filename); window.setTimeout(() => URL.revokeObjectURL(url), 1000); } catch { setMessage("图片下载失败，请稍后重试。"); } }
