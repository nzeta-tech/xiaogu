...
export function XiaohongshuStudioPageClient({ app }: { app: CreationApp }) {
  ...
  function download(card: Card, index: number) { const anchor = document.createElement("a"); anchor.href = card.url; anchor.download = `${title}-小红书配图-${index + 1}.png`; anchor.rel = "noopener"; document.body.appendChild(anchor); anchor.click(); anchor.remove(); }
  async function downloadImagePack() { const images = [headImage, ...cards].filter((item): item is Card => Boolean(item)); if (!images.length) return; setMessage(""); try { await Promise.all(images.map(async (image, index) => { const response = await fetch(image.url); if (!response.ok) throw new Error("图片下载失败"); const blob = await response.blob(); const url = URL.createObjectURL(blob); const anchor = document.createElement("a"); anchor.href = url; anchor.download = `${title}-小红书${index === 0 ? "头图" : `章节配图-${index}`}.png`; document.body.appendChild(anchor); anchor.click(); anchor.remove(); window.setTimeout(() => URL.revokeObjectURL(url), 1000); })); setMessage(`已开始下载 ${images.length} 张图片。`); } catch { images.forEach((image, index) => download(image, index)); setMessage("已逐张开始下载图片。"); } }
