...
export function XiaohongshuStudioPageClient({ app }: { app: CreationApp }) {
  ...
  async function download(card: Card, index: number) { try { const source = card.url.startsWith("/") || card.url.startsWith(window.location.origin) ? card.url : apiPath(`/api/assets/image-proxy?url=${encodeURIComponent(card.url)}`); const response = await fetch(source); if (!response.ok) throw new Error("图片下载失败"); const blob = await response.blob(); const url = URL.createObjectURL(blob); const anchor = document.createElement("a"); anchor.href = url; anchor.download = `${title}-小红书${index === 0 ? "头图" : `章节配图-${index}`}.png`; document.body.appendChild(anchor); anchor.click(); anchor.remove(); window.setTimeout(() => URL.revokeObjectURL(url), 1000); } catch { setMessage("图片下载失败，请稍后重试。"); } }
