...
export function PptMakerPageClient() {
  ...
  async function importFile(file: File | undefined) {
    ...
    try {
      ...
      setSource(payload.text); setMessage(payload.text.length > 60000 ? `已提取《${file.name}》，共 ${payload.text.length} 字，超过 60,000 字上限；请精简或拆分后提交，系统不会静默截断。` : `已提取《${file.name}》的内容。你可以继续编辑，或直接开始生成。`);
  ...
  return <main className="pptStudio pageStack">
    ...
    <div className="pptStudioGrid"><section className="pptComposer"><div className="pptSectionHeading"><div><span>01 · 内容</span><h2>先给我一个清晰的方向</h2><p>主题和资料至少填写一项。资料越完整，内容越贴近真实沟通。</p></div><span className={`pptReadiness ${inputReady ? "isReady" : ""}`}>{inputReady ? "内容已就绪" : "等待输入"}</span></div>
      ...
      <label className="pptField"><span>补充资料 <em>可选</em></span><div className="pptVoiceField"><textarea value={source} onChange={(event) => setSource(event.target.value)} placeholder="可粘贴文章、口播稿、方案要点或会议纪要。不要提供客户身份证号、保单号等敏感信息。" rows={8} /><button className={listeningField === "source" ? "isListening" : ""} type="button" onClick={() => toggleVoiceInput("source")} aria-label="语音输入补充资料">{listeningField === "source" ? "■ 停止" : "◉ 语音输入"}</button></div><small>{listeningField === "source" ? "正在聆听，请开始说话…" : `${contentSummary} · ${source.length.toLocaleString()}/60,000 字${source.length > 60000 ? " · 已超出上限，不会静默截断" : ""}`}</small></label>
