...
export function ProfilePageClient({ skillScope = "personal", trainingOnly = false }: { skillScope?: "personal" | "platform"; trainingOnly?: boolean } = {}) {
  ...
  const [wechatChannelDraft, setWechatChannelDraft] = useState({ channelId: "", limit: "10" as "10" | "20" | "50" | "100" | "all" });
  ...
  async function discoverWechatChannelWorks() {
    ...
    try {
      ...
      const payload = await response.json() as { candidates?: WechatChannelCandidate[]; authorName?: string; requestCount?: number; pageCount?: number; cacheHitCount?: number; providerRequestCount?: number; error?: string };
      ...
      setNotice(`已加载${payload.authorName ? `「${payload.authorName}」` : "该视频号"} ${payload.candidates.length} 条作品，共使用 ${payload.pageCount ?? payload.requestCount ?? 0} 页；缓存命中 ${payload.cacheHitCount ?? 0} 项，TikHub 实际调用 ${payload.providerRequestCount ?? 0} 次。`);
