...
export function deterministicWorkbuddyRoute(objective: string, hasContext = false): WorkbuddyRequestRoute | null {
  ...
  if (/(?:我想|我要|需要|请|帮我|给我|替我|开始|进入).{0,16}(?:写|做|制作|生成|创作|分析).{0,10}(?:流量型.{0,4})?口播(?:稿|文案)?|(?:口播文案|口播稿).{0,8}(?:流量型|创作|生成|选题流程)/.test(text)) {
    return { mode: "capability", intent: text.slice(0, 160), targetCapabilityId: "app.traffic-copy", requiresFreshInformation: false, rationale: "用户明确要求创作流量型口播；先进入应用参数页，检索由选题竞技场统一完成" };
  }
