...
function buildStandaloneModePrompt(
  ...
  const sourceFidelityInstructions = variant === "traffic"
    ? [
        "素材保真要求（优先级高于通用传播结构和篇幅建议）：",
        "1. 先在心中整理素材里的事实清单：产品/服务对象、年龄或人群、金额和领取节奏、收益或数据口径、合同规则、适用范围、限制条件与行动引导。不得把这些具体信息整体改写成泛泛的养老、理财或保险观点。",
        "2. 素材含有产品规则、数字、收益口径、年龄、地区、服务权益或对比时，正文必须保留支撑核心结论的 3-5 个关键信息及其条件；数字、单位、年龄和时间不得擅自改写。篇幅不够时压缩解释，不得删除全部事实后只保留抽象结论。",
        "3. 可以重组顺序和压缩口语，但不能把明确的产品推荐稿改写成中性科普、把产品规则改成“有的产品”，或写出与原素材立场相反的劝退结论。",
        "4. 涉及收益、领取、现金价值、身故、照护、投保或销售范围时，只能转述素材已经提供的事实，并同时保留其适用条件；对未提供或无法确认的内容标注“以合同/实际规则为准”，不要自行补充或承诺。",
        "5. 成稿不得出现“素材”“原文”“用户提供”“案例被保留”“根据输入”等内部处理痕迹；应把事实自然写进正文，例如直接使用“以60岁一次交100万为例”，不要交代信息来自哪里。",
        "6. 素材信息密度高时，正文可扩展至 800 字以内，以保证关键事实不失真；优先选取最能支撑结论的数字节点，不要机械罗列全部数据。",
        "",
      ]
    : [];
  const outputContract = variant === "traffic"
    ? [
        ...
        "【金句与互动】",
        "【标题建议（3个）】",
        "正文默认控制在 300-500 字；素材明显适合金句/类比型时可压缩至 80-150 字，明显适合个人经历或案例型时可扩展至 600 字以内。",
        "先选择结果、原话、场景、问题、对比或身份中的一种钩子；不得机械使用反常识开头。",
      ]
    : [
...
function getLeadCopyTargetSpec(target: string, variant: MultiChannelCopyVariant): LeadCopyTargetSpec | null {
  if (target === "video_batch") {
    ...
    return {
      ...
      instructions: [
        "必须输出且只输出 3 篇：",
        variant === "traffic" ? "版本一｜场景故事版" : "版本一｜反常识版",
        variant === "traffic" ? "版本二｜问题回应版" : "版本二｜直击痛点版",
        variant === "traffic" ? "版本三｜观点类比版" : "版本三｜故事共鸣版",
        "三篇都要有完整引流闭环，但切入角度、论证路径和互动关键词不能重复；语言要能直接口播。",
        ...(variant === "traffic" ? [
          "每篇先从结果、原话、场景、问题、对比或身份中选择一种钩子；前两句必须给出具体人、具体场景、具体结果、具体疑问或具体对比中的至少一项。",
          "按“钩子 -> 观众代入 -> 常见误解/真实矛盾 -> 具体场景或事实 -> 新判断 -> 金句收束 -> 互动”推进。每篇只保留一个核心判断，不得把三种版本写成同一篇换标题。",
          "至少给出一个普通人看得见的生活画面；金句短、可复述、不喊口号。互动动作可以是提问、评论关键词、分享给家人或私信沟通，不强迫销售承接。",
        ] : []),
      ],
    };
  }
  ...
  if (target === "redbook_batch") {
    ...
    return {
      ...
      instructions: [
        ...
        ...(variant === "traffic" ? ["每篇都要有明确立场、具体场景、读者代入和 3 个标题建议；可使用结果、原话、问题、对比或身份钩子，不写新闻摘要或模板化反常识开头。"] : []),
      ],
    };
  }
  ...
  if (target === "wechat_batch") {
    ...
    return {
      ...
      instructions: [
        ...
        ...(variant === "traffic" ? ["每篇都要用事实、迁移逻辑与家庭风险场景推进，保留一个明确判断和一个可执行启发；开头使用具体场景、问题、原话、结果或对比，而不是空泛论点。"] : []),
      ],
    };
  }
...
function buildMarketingTarget(heading: string, label: string, channelInstruction: string): LeadCopyTargetSpec {
  return {
    ...
    instructions: [
      ...
      "讲产品：从客户疑问或误解切入，讲清解决什么问题、不能解决什么问题、适合谁和不适合谁；不堆卖点，不先报收益或金额。",
      "讲方案：按“目标 -> 约束 -> 选择 -> 复核”讲清客户为什么这样规划；年龄、预算、现金流和领取时间等数字必须来自素材或标注待核实。",
      "讲案例：从人物处境或一句原话开场，写清犹豫、判断和现实问题；素材没有真实案例时只能使用明确标注的典型情境，不得伪造客户和结果。",
      "讲观念：围绕一个常见问题、对比或误解展开，最后给出可复述的决策原则，不写产品口号。",
      "四篇都按“人和处境 -> 决策矛盾 -> 专业判断 -> 边界说明 -> 轻承接”推进；开头优先使用结果、原话、场景、问题或对比。",
      "涉及健康告知、既往症、等待期、续保、免责、承保、理赔、费率、收益或领取时必须说明边界，不得只写好处，也不得使用稳赚、保证领取等绝对表达。",
    ],
  };
}
...
function getVariantPrinciples(variant: MultiChannelCopyVariant) {
  if (variant === "traffic") {
    return [
      ...
      "2. 每篇先选择结果、原话、场景、问题、对比或身份中的一种钩子；不强制反常识，不使用低质标题党和模板开头。",
      "3. 主体按“观众代入 -> 常见误解/真实矛盾 -> 具体场景或事实 -> 新判断”推进；每篇至少有一个普通人代入场景和一个明确结论。",
      "4. 结尾用一句短、可复述的金句收束，再给自然互动动作；可以有情绪张力，但不得编造事实、承诺收益或理赔，也不得把推测写成确定结论。",
    ];
  }
  if (variant === "marketing") {
    return [
      ...
      "3. 每篇从人和处境开始，通过决策矛盾和专业判断建立信任，同时讲清价值与规则边界；不要写硬广口号或先报产品收益。",
      "4. 互动动作必须具体但低压，可使用私信关键词、预约梳理或领取清单；不得承诺一定承保、一定理赔、确定收益或保证领取。",
      "5. 素材缺少案例事实时改用明确标注的典型情境，不得虚构人物、结果、金额、收益或产品规则。",
    ];
  }
  return [
