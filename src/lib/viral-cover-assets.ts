...
// 120-byte transparent PNGs are returned by some blocked upstream pages. They
// are technically valid images but never a usable creator-facing cover.
...
export async function storeViralCover(input: { contentId: string; sourceUrl: string; refererUrl?: string }) {
  ...
  // 二创链路下载的抖音封面会被暂存为站内媒体地址。该媒体端点仅允许
  // 本地 Agent 读取；仅在地址严格等于本机执行器的媒体路径时转发凭证。
