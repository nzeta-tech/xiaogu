...
export function validateUploadAssembly(value: unknown){
  ...
  if(!v||!uuid.test(v.jobId||'')||!['output','presenter_master'].includes(v.kind||'')||!Array.isArray(v.parts)||v.parts.length<1||v.parts.length>512||new Set(v.parts).size!==v.parts.length||v.parts.some(id=>!uuid.test(id))||!Number.isSafeInteger(v.size)||v.size!<1||v.size!>500*1024*1024||!/^[a-f0-9]{64}$/.test(v.sha256||'')||!['video/mp4','video/quicktime','video/webm'].includes(v.contentType||'')||typeof v.fileName!=='string'||v.fileName.length>240)throw Error('invalid_upload_assembly');
