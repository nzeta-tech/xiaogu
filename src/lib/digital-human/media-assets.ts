...
type MediaKind = "source" | "preview" | "presenter_master" | "output" | "voice_source";
...
export async function storeDigitalHumanMedia(input: { userId: string; digitalHumanId?: string; videoJobId?: string; kind: MediaKind; bytes: Buffer; contentType: string; fileName: string; sourceUrl?: string }) {
  ...
  const useLocal = settings.digitalHuman.localDiskEnabled;
...
export async function storeDigitalHumanMediaStream(input: { userId: string; digitalHumanId?: string; videoJobId: string; kind: "presenter_master" | "output"; body: ReadableStream<Uint8Array>; contentLength: number; contentType: string; fileName: string; sourceUrl?: string }) {
  const settings = await tryGetSystemSettings();
  if (!settings.digitalHuman.localDiskEnabled) throw new Error("服务器媒体磁盘尚未启用");
  ...
  let size=0; const hash=createHash("sha256"); const meter=new Transform({transform(chunk,_encoding,callback){size+=chunk.length;if(size>2*1024*1024*1024)return callback(new Error("媒体文件超过 2 GB 安全上限"));hash.update(chunk);callback(null,chunk)}});
  try {
