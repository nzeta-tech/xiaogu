...
import { buildViralCoverUrl } from "./viral-cover-url";
...
function databaseRowToExample(row: Awaited<ReturnType<typeof tryListPublishedViralContents>>[number], cachedUrls: Set<string>): ViralExample {
  return {
    ...
    excerpt: row.summary || undefined, thumbnailUrl: row.has_local_cover ? buildViralCoverUrl(row.id, row.cover_sha256) : row.thumbnail_url ?? undefined, mediaUrl: row.media_url ?? undefined,
