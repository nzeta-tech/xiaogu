...
import { workflowContractForApp } from "@/lib/apps/workflow-contract";
...
export async function GET(_: Request, context: { params: Promise<{ slug: string }> }) {
  ...
  return Response.json({ app, workflow: workflowContractForApp(app) });
}
...
function buildWechatStudioPrompt(values: Record<string, FieldValue>, caseContext: string[]) {
  ...
  return [
    ...
    "6. 产品、案例与数据不确定时明确提示需要核验；任务涉及保险时，不承诺收益、承保或理赔。",
    `目标读者：${audience}。`,
    `文章气质：${tone}。`,
    "用户的真实素材与要求：",
    topic,
...
function buildWriteCopyPrompt(
  ...
  const lines = [
    "你现在在执行小谷应用：写文案。",
    "你要扮演一个服务财经、财富、保险或其他专业受众的顾问型内容创作者，并保持用户原题领域。",
    ...
    "统一写作原则：",
    "1. 先讲人或讲现象，再讲你的判断，最后自然落到与原题相关的机制、决策或服务价值；不得强行落到保险。",
    ...
    "9. 重点表达的不是产品，而是：清醒判断、长期主义、专业服务和对真实问题的理解。",
    "10. CTA 要轻，像“评论区留关键词”“你可以来问我”“我们私下聊聊”，不要硬逼单。",
    "11. 不得编造事实或夸大产品；涉及保险时不得承诺收益、承保或理赔，不恐吓客户。",
...
function buildIpPositioningPrompt(
  ...
  const lines = [
    ...
    "你现在要完成的是：基于完整人设画像和本轮提交信息，输出一份可长期使用、可落地执行的财经、财富、保险或其他专业创作者个人IP定位分析。",
    ...caseContext,
    caseContext.length > 0 ? "" : "",
    hint,
    "",
  ...
  if (target === "video_script") {
    return {
      ...
      instructions: [
        ...
        "3. 按“钩子 -> 代入 -> 转折 -> 真实细节/场景 -> 核心判断 -> 情绪收束 -> 轻承接”推进。先讲人和处境，再自然落到原题相关的机制、家庭决策或专业服务。",
