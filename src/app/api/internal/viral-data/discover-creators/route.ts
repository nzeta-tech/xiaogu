import { runViralCreatorDiscovery } from "@/lib/viral-creator-task";
...
export async function POST(request: Request) {
  ...
  const result = await runViralCreatorDiscovery({ force: Boolean(payload.force), trigger: payload.trigger ?? "creator-worker" });
