...
export async function POST(request: Request) {
  ...
  const body = await request.json().catch(() => ({})) as { jobId?: string; progress?: number; stage?: string; creativeSummary?: string[] };
  ...
  const updated = await updateDigitalHumanVideoJobById(body.jobId, { progress, request: { stage: String(body.stage || "processing").slice(0, 80), creative_summary: Array.isArray(body.creativeSummary) ? body.creativeSummary.map(String).slice(0, 6) : [] } });
