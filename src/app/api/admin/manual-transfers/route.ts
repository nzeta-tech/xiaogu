...
export async function PATCH(request: Request) {
  ...
  const receipt = await query<{ receipt_url: string }>("select receipt_url from payment_manual_reviews where order_id=$1", [order.id]);
  if (!receipt.rows[0]?.receipt_url.trim()) return Response.json({ error: "请先补充转账凭据，核验实际收款后再通过审核。" }, { status: 400 });
  ...
  await recordVerifiedManualPayment(order.id);
